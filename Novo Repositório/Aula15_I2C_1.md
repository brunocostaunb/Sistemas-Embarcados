...
´´´ C #include <stdio.h> #include <stdlib.h> #include <signal.h> #include <fcntl.h> #include <linux/ioctl.h> #include <linux/i2c-dev.h>

int i2c_fd; void ctrl_c(int sig) { close(i2c_fd); exit(-1); }

int main(void) { unsigned char begin = 0x55, msp430_ret_1, msp430_ret_2, rpi_addr = 0xDA, slave_addr=0x0F; long int soma = 0; int i = 0; float media = 0; signal(SIGINT, ctrl_c); i2c_fd = open("/dev/i2c-1", O_RDWR); ioctl(i2c_fd, I2C_SLAVE, slave_addr);
